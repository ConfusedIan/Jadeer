...
(function () {
  ...
  s.textContent = `
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav-item.active { color: white; background: rgba(79,70,229,0.2); border-left-color: #4f46e5; }
    ...
    .logout-btn:hover { color: #94a3b8; border-color: rgba(255,255,255,0.2); }
    ...
    .btn-ghost { background: white; color: #374151; border: 1px solid #e2e8f0; }
    ...
    .item-actions { display: flex; gap: 6px; flex-shrink: 0; padding-top: 2px; }
    ...
    /* ── Empty ── */
    .empty { text-align: center; padding: 28px 16px; color: #94a3b8; font-size: 14px; }
    ...
    .spinner.dark { border-color: rgba(79,70,229,0.2); border-top-color: #4f46e5; }
...
function initPage(active) {
  if (!getToken()) { window.location.href = '/dev/index.html'; return; }

  const nav = document.createElement('nav');
  nav.className = 'sidebar';
  nav.innerHTML = `
    <div class="brand">Jadeer <small>Platform</small></div>
    <div class="nav-section">
      <div class="nav-section-label">My Profile</div>
      <a href="/dev/profile.html" class="nav-item ${active === 'profile' ? 'active' : ''}">👤 Profile</a>
      <a href="/dev/cv.html" class="nav-item ${active === 'cv' ? 'active' : ''}">📄 CV Builder</a>
    </div>
    <div class="nav-section">
...
function showAlert(id, msg, type = 'error') {
  ...
  setTimeout(() => { if (el) el.className = `alert alert-${type}`; }, 4000);
}
...
function switchTab(tabsId, panePrefix, name) {
  ...
  document.getElementById(`${panePrefix}${name}`)?.classList.add('active');
}
